...
class Task {
    ...
    bool astar(const Cell& start_state) {
        ...
        std::set<std::pair<double, Cell>, decltype(comparator)> open(comparator);
                ...
                for (auto& c : expand(s)) {
                    if (g[c] == DBL_MAX || g[s] + 1.0 < g[c]) {
                        ...
                        if (!(closed.find(c) != closed.end()
                                || std::find_if(open.begin(), open.end(), [&c](auto& p) {
                                       return p.second == c;
                                   }) != open.end())) {
