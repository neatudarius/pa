/*
    ...
    d[i][i] = 0 (evident)
    d[i][j] = oo, daca i != j si nu exista muchia i->j (inseamna ca nu am descoperit inca drum de la i la j)

    in final d[i][j] = numarul minim de muchii de parcurs de la i ca sa ajungem la j
    daca d[i][j] < oo, atunci exista drum de lungime d[i][j] de la i la j,
    deci clar ca j este accesibil din i

    (daca nu ne intereseaza efectiv valorile drumurilor, putem lucra doar cu 0 si 1 (exista sau nu exista drum))
*/
...
void solve() {
   cout << "initial: 1 => drum de lungime 1 de la i la j; 0 pt muchie i,i; -1 nu exista drum de la i la j\n";
   print_output();

   RoyFloyd();

   cout << "final: x => drum de lungime x de la i la j; 0 pt muchie i,i; -1 nu exista drum de la i la j\n";
   print_output();
}
